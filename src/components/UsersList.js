...
class UsersList extends Component {
  ...
  fetchUsers = () => {
    fetch("http://mgt2.pnu.ac.th/php-react/all-users.php")
      .then((response) => {
        response.json().then(
          function (data) {
            if (data.success === 1) {
  ...
  handleUpdate = (id) => {
    Axios.post("http://mgt2.pnu.ac.th/php-react/update-user.php", {
      id: id,
  ...
  handleDelete = (id) => {
    ...
    Axios.post("http://mgt2.pnu.ac.th/php-react/delete-user.php", {
      id: id,
    })
      .then(({ data }) => {
        if (data.success === 1) {
  ...
  render() {
    let allUsers = this.state.users.map(
      ({ id, user_name, user_email, isEditing }, index) => {
